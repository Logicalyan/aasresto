    ...
    <main class="p-4 md:ml-64 h-auto pt-20">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
            <button data-modal-target="modal"
                ...
                class="block text-white bg-green-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center transition-all duration-700 ease-in-out"
                type="button">
                Tambah
            </button>
            <div id="modal" tabindex="-1" aria-hidden="true"
                            class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full transition-all duration-700 ease-in-out">
                                ...
                                <div class="relative bg-white rounded-lg shadow">
                                    ...
                                    <div class="p-4 md:p-5">
                                        ...
                                        <form class="grid gap-4 sm:grid-cols-2 sm:gap-6" enctype="multipart/form-data"
                                            action="{{ route('table.store') }}" method="POST">
                                            @csrf

                                            <div class="cm:cols-span-2">
                    ...
                    @foreach ($tables as $table)
                        <tr class="bg-white border-b">
                            ...
                            <td class="px-6 py-4 flex gap-2">
                                ...
                                <button data-modal-target="modal{{ $table->id }}"
                                    data-modal-toggle="modal{{ $table->id }}"
                                ...
                                <button data-modal-target="popup-modal{{ $table->id }}"
                                    data-modal-toggle="popup-modal{{ $table->id }}"
                        ...
                        <!-- Main  -->
                        <div id="modal{{ $table->id }}" tabindex="-1" aria-hidden="true"
                            ...
                            <div class="relative p-4 w-full max-w-md max-h-full">
                                ...
                                <div class="relative bg-white rounded-lg shadow">
                                    ...
                                    <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t">
                                        <h3 class="text-xl font-semibold text-gray-900">
                                            Ubah data tabel {{ $table->id }}
                                        </h3>
                                        <button type="button"
                                            class="end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center"
                                            data-modal-hide="modal{{ $table->id }}">
                                            <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                                                fill="none" viewBox="0 0 14 14">
                                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
                                                    stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
                                    ...
                                    <div class="p-4 md:p-5">
                                        ...
                                        <form class="grid gap-4 sm:grid-cols-2 sm:gap-6" enctype="multipart/form-data"
                                            action="{{ route('table.update', $table->id) }}" method="POST">
                                            @csrf
                                            @method('PUT')
                                            ...
                                            <div class="w-full">
                                                <label for="table_name"
                                                    ...
                                                    category</label>
                                                <input type="text" name="name" id="table_name"
                                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                                    placeholder="Ubah Table" required
                        ...
                        <div id="popup-modal{{ $table->id }}" tabindex="-1"
                            class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                            <div class="relative p-4 w-full max-w-md max-h-full">
                                <div class="relative bg-white rounded-lg shadow">
                                    <button type="button"
                                        class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center"
                                        data-modal-hide="popup-modal{{ $table->id }}">
                                        <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                                            fill="none" viewBox="0 0 14 14">
                                    ...
                                    <div class="p-4 md:p-5 text-center">
                                        ...
                                        <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Yakin ingin menghapus {{ $table->name }}</h3>
                                        <form action="{{ route('table.destroy', $table->id) }}" method="post">
                                            @csrf
                                            @method('DELETE')

                                            <button data-modal-hide="popup-modal{{ $table->id }}" type="submit"
                                                class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center">
                                                Ya
                                            </button>
                                        </form>
                                        <button data-modal-hide="popup-modal{{ $table->id }}" type="button"
