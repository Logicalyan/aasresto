<main class="p-4 md:ml-64 peer-checked:ml-16 h-auto pt-20">
    ...
    <form class="md:w-full mx-auto" method="GET" action="{{ route('home') }}">
        ...
        <div class="relative">
            ...
            <input type="text" name="search" id="default-search" value="{{ app('request')->input('search') }}"
                class="block w-full p-4 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500"
                placeholder="Cari Nama Menu..." required />
            <button
            ...
            @foreach ($menus as $item)
                <div class="h-auto p-2 bg-white border rounded-lg">
                    <figure class="w-64 h-36">
                        <img class="h-full w-full border rounded-lg" src="{{ asset('/storage/menus/' . $item->image) }}"
                            alt="product image" />
                    </figure>
